...
vector<std::pair<Eigen::Vector4f, float>> LinearEvaluator::GetFeatures(
  ...
  vector<std::pair<Eigen::Vector4f, float>> features(paths.size());
  ...
  // float best_cost =
  //     FLAGS_dw * dist_to_goal[best_i] + FLAGS_fw * best->Length() +
  //     FLAGS_ow * option_clearance[best_i] + FLAGS_cw * clearance[best_i] +
  ...
  // const float cost =
  //     FLAGS_dw * dist_to_goal[i] + FLAGS_fw * paths[i]->Length() +
  //     FLAGS_ow * option_clearance[i] + FLAGS_cw * clearance[i] +
...
float LinearEvaluator::GetClearanceWeight() { return FLAGS_cw; }

float LinearEvaluator::GetDistanceWeight() { return FLAGS_dw; }

float LinearEvaluator::GetOptionClearanceWeight() { return FLAGS_ow; }

float LinearEvaluator::GetFreePathWeight() { return FLAGS_fw; }
