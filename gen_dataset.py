...
def extract_joystick_data(subfolder):
    data_frame = pd.read_csv(os.path.join(subfolder,"_slash_joystick.csv"))
...
def vesc_data(subfolder):
    data = pd.read_csv(os.path.join(subfolder,"_slash_vesc_drive.csv"))

    secs = data["secs"].to_numpy()
    nsecs = data["nsecs"].to_numpy()
    times = secs + nsecs / 1e9 - secs[0]

    vels = data["x"]
    ang_vels = data["z.1"]

    curvatures = ang_vels / vels

    return (times, curvatures, vels)


def range_data(subfolder):
    scan = pd.read_csv(os.path.join(subfolder,"_slash_scan.csv"))
...
def write_train_data(n, filename, subfolder):
    scan_data = range_data(subfolder)
    joy_data = extract_joystick_data(subfolder)
    # vesc_data = vesc_data()
    ...
    print("writing data to " + os.path.join(subfolder,filename))
    training_data.to_csv(os.path.join(subfolder,filename))

parser = argparse.ArgumentParser()
parser.add_argument("-d", "--directory", help = "directory with bag file csvs")
args = parser.parse_args()

write_train_data(3600, "dataset.csv", args.directory)
